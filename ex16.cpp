...
int main( int argc, char* argv[] )
{
    ...
    const char* mesh_file = "/home/dimiao/repo/mfem/data/star.mesh";
    int ref_levels = 2;
    int order = 2;
    ...
    ParaViewDataCollection visit_dc( "Example16", mesh );
    visit_dc.SetPrefixPath( "ParaView" );
    visit_dc.SetLevelsOfDetail( order );
    visit_dc.SetDataFormat( VTKFormat::BINARY );
    visit_dc.SetHighOrderOutput( true );
    ...
    if ( visualization )
    {
        ...
        if ( !sout )
        {
            cout << "Unable to connect to GLVis server at " << vishost << ':' << visport << endl;
            visualization = false;
