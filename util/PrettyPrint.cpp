...
SetColor::SetColor( Color fg, Color bg ) : fg( fg ), bg( bg )
{
}
...
{
    return "\033[" + std::to_string( abs( lines_down ) ) + ( lines_down < 0 ? 'A' : 'B' ) + "\033[" +
           std::to_string( abs( columns_right ) ) + ( columns_right < 0 ? 'D' : 'C' );
}
