...
{
public:
    void operator()( int32_t sortType, bool isRotation, Eigen::Matrix<T, 3, 1>& eval, Eigen::Matrix<T, 3, 3>& evec )
    {
        if ( sortType != 0 )
        {
            ...
            Eigen::Matrix<T, 3, 1> unorderedEVal = eval;
            Eigen::Matrix<T, 3, 3> unorderedEVec = evec;
            for ( size_t j = 0; j < 3; ++j )
            {
                size_t i = index[j];
                eval( j ) = unorderedEVal( i );
                evec.col( j ) = unorderedEVec.col( i );
            }
        }
...
public:
                        ...
                        Eigen::Matrix<T, 3, 3>& evec ) const
    {
        ...
        Eigen::Matrix<T, 3, 3> Q{ { c, s, zero }, { s, -c, zero }, { zero, zero, one } };

        // The smallest subnormal number is 2^{-alpha}. The value alpha is
        // 149 for 'float' and 1074 for 'double'.
        int32_t constexpr alpha = std::numeric_limits<autodiff::detail::NumericType<T>>::digits -
                                  std::numeric_limits<autodiff::detail::NumericType<T>>::min_exponent;
        int32_t i = 0, imax = 0, power = 0;
        T c2 = zero, s2 = zero;

        if ( autodiff::detail::abs( b12 ) <= autodiff::detail::abs( b01 ) )
        {
            ...
            (void)std::frexp( autodiff::detail::val( b12 ), &power );
            ...
            for ( i = 0; i < imax; ++i )
            {
                ...
                s = autodiff::detail::sqrt( half * ( one - c2 ) );
                c = half * s2 / s;

                // Update Q <- Q * G.
                for ( size_t r = 0; r < 3; ++r )
                {
                    term0 = c * Q( 0, r ) + s * Q( 1, r );
                    term1 = Q( 2, r );
                    term2 = c * Q( 1, r ) - s * Q( 0, r );
                    Q( 0, r ) = term0;
                    Q( 1, r ) = term1;
                    Q( 2, r ) = term2;
                }
        ...
        else
        {
            ...
            (void)std::frexp( autodiff::detail::val( b01 ), &power );
            imax = 2 * ( power + alpha + 1 );
            for ( i = 0; i < imax; ++i )
            {
                // Compute the Givens reflection
                ...
                for ( size_t r = 0; r < 3; ++r )
                {
                    term0 = c * Q( 1, r ) + s * Q( 2, r );
                    term1 = Q( 0, r );
                    term2 = c * Q( 2, r ) - s * Q( 1, r );
                    Q( 0, r ) = term0;
                    Q( 1, r ) = term1;
                    Q( 2, r ) = term2;
                }
...
private:
    ...
    {
        T const zero = static_cast<T>( 0 );
        T length = autodiff::detail::sqrt( u * u + v * v );
        if ( length > zero )
        {
            c = u / length;
            s = v / length;
    ...
    static bool Converged( bool aggressive, T const& diagonal0, T const& diagonal1, T const& superdiagonal )
    {
        if ( aggressive )
        {
        ...
        else
        {
            ...
            T sum = autodiff::detail::abs( diagonal0 ) + autodiff::detail::abs( diagonal1 );
            return sum + autodiff::detail::abs( superdiagonal ) == sum;
        }
    }
};
...
public:
    ...
    void operator()( T a00, T a01, T a02, T a11, T a12, T a22, int32_t sortType, Eigen::Matrix<T, 3, 1>& eval, Eigen::Matrix<T, 3, 3>& evec ) const
    {
        ...
        T max0 = autodiff::detail::max( autodiff::detail::abs( a00 ), autodiff::detail::abs( a01 ) );
        T max1 = autodiff::detail::max( autodiff::detail::abs( a02 ), autodiff::detail::abs( a11 ) );
        T max2 = autodiff::detail::max( autodiff::detail::abs( a12 ), autodiff::detail::abs( a22 ) );
        T maxAbsElement = autodiff::detail::max( autodiff::detail::max( max0, max1 ), max2 );
        if ( maxAbsElement == (T)0 )
        {
            // A is the zero matrix.
            eval.setZero();
            evec = Eigen::Matrix<T, 3, 3>::Identity();
            return;
        }
        ...
        if ( norm > (T)0 )
        {
            ...
            T p = autodiff::detail::sqrt( ( b00 * b00 + b11 * b11 + b22 * b22 + norm * (T)2 ) / (T)6 );
            ...
            T halfDet = det * (T)0.5;
            halfDet = autodiff::detail::min( autodiff::detail::max( halfDet, (T)-1 ), (T)1 );
            ...
            // 2*pi/3.
            T angle = autodiff::detail::acos( halfDet ) / (T)3;
            T const twoThirdsPi = (T)2.09439510239319549;
            T beta2 = autodiff::detail::cos( angle ) * (T)2;
            T beta0 = autodiff::detail::cos( angle + twoThirdsPi ) * (T)2;
            T beta1 = -( beta0 + beta2 );
            ...
            if ( halfDet >= (T)0 )
            {
                ComputeEigenvector0( a00, a01, a02, a11, a12, a22, eval( 2 ), evec.col( 2 ) );
                ComputeEigenvector1( a00, a01, a02, a11, a12, a22, evec.col( 2 ), eval( 1 ), evec.col( 1 ) );
                evec.col( 0 ) = evec.col( 1 ).cross( evec.col( 2 ) );
            }
            else
            {
                ComputeEigenvector0( a00, a01, a02, a11, a12, a22, eval( 0 ), evec.col( 0 ) );
                ComputeEigenvector1( a00, a01, a02, a11, a12, a22, evec.col( 0 ), eval( 1 ), evec.col( 1 ) );
                evec.col( 1 ) = evec.col( 0 ).cross( evec.col( 1 ) );
            }
        }
        else
        {
...
private:
    void ComputeOrthogonalComplement( Eigen::Ref<const Eigen::Vector<T, 3>> const& W,
                                      Eigen::Ref<Eigen::Vector<T, 3>> U,
                                      Eigen::Ref<Eigen::Vector<T, 3>> V ) const
    {
        ...
        // computing invLength.
        T invLength;
        if ( autodiff::detail::abs( W( 0 ) ) > autodiff::detail::abs( W( 1 ) ) )
        {
            // The component of maximum absolute value is either W(0)
            // or W(2).
            invLength = (T)1 / autodiff::detail::sqrt( W( 0 ) * W( 0 ) + W( 2 ) * W( 2 ) );
        ...
        else
        {
            ...
            // or W(2).
            invLength = (T)1 / autodiff::detail::sqrt( W( 1 ) * W( 1 ) + W( 2 ) * W( 2 ) );
            U << (T)0, +W( 2 ) * invLength, -W( 1 ) * invLength;
        }
        V = W.cross( U );
    }

    void ComputeEigenvector0( T a00, T a01, T a02, T a11, T a12, T a22, T eval0, Eigen::Ref<Eigen::Vector<T, 3>> evec0 ) const
    {
        ...
        // rows.
        Eigen::Vector<T, 3> row0{ a00 - eval0, a01, a02 };
        Eigen::Vector<T, 3> row1{ a01, a11 - eval0, a12 };
        Eigen::Vector<T, 3> row2{ a02, a12, a22 - eval0 };
        Eigen::Vector<T, 3> r0xr1 = row0.cross( row1 );
        Eigen::Vector<T, 3> r0xr2 = row0.cross( row2 );
        Eigen::Vector<T, 3> r1xr2 = row1.cross( row2 );
        ...
        if ( imax == 0 )
        {
            evec0 = r0xr1 / autodiff::detail::sqrt( d0 );
        }
        else if ( imax == 1 )
        {
            evec0 = r0xr2 / autodiff::detail::sqrt( d1 );
        }
        else
        {
            evec0 = r1xr2 / autodiff::detail::sqrt( d2 );
        }
    }
    ...
    void ComputeEigenvector1( T a00,
                              ...
                              T a12,
                              T a22,
                              Eigen::Ref<const Eigen::Vector<T, 3>> const& evec0,
                              T eval1,
                              Eigen::Ref<Eigen::Vector<T, 3>> evec1 ) const
    {
        // Robustly compute a right-handed orthonormal set
        // { U, V, evec0 }.
        Eigen::Vector<T, 3> U, V;
        ...
        Eigen::Vector<T, 3> AU{ a00 * U( 0 ) + a01 * U( 1 ) + a02 * U( 2 ), a01 * U( 0 ) + a11 * U( 1 ) + a12 * U( 2 ),
                                a02 * U( 0 ) + a12 * U( 1 ) + a22 * U( 2 ) };

        Eigen::Vector<T, 3> AV{ a00 * V( 0 ) + a01 * V( 1 ) + a02 * V( 2 ), a01 * V( 0 ) + a11 * V( 1 ) + a12 * V( 2 ),
        ...
        T absM00 = autodiff::detail::abs( m00 );
        T absM01 = autodiff::detail::abs( m01 );
        T absM11 = autodiff::detail::abs( m11 );
        T maxAbsComp;
        if ( absM00 >= absM11 )
        {
            maxAbsComp = autodiff::detail::max( absM00, absM01 );
            if ( maxAbsComp > (T)0 )
            {
                if ( absM00 >= absM01 )
                {
                    m01 /= m00;
                    m00 = (T)1 / autodiff::detail::sqrt( (T)1 + m01 * m01 );
                    m01 *= m00;
                }
                else
                {
                    m00 /= m01;
                    m01 = (T)1 / autodiff::detail::sqrt( (T)1 + m00 * m00 );
                    m00 *= m01;
                }
                evec1 = m01 * U - m00 * V;
            }
        ...
        {
            maxAbsComp = autodiff::detail::max( absM11, absM01 );
            if ( maxAbsComp > (T)0 )
            {
                if ( absM11 >= absM01 )
                {
                    m01 /= m11;
                    m11 = (T)1 / autodiff::detail::sqrt( (T)1 + m01 * m01 );
                    m01 *= m11;
                }
                else
                {
                    m11 /= m01;
                    m01 = (T)1 / autodiff::detail::sqrt( (T)1 + m11 * m11 );
                    m11 *= m01;
                }
                evec1 = m11 * U - m01 * V;
            }
