...
public:
    JacobianPreconditioner( Array<FiniteElementSpace*>& fes, SparseMatrix& mass, Array<int>& offsets );
...
public:
    RubberOperator( Array<FiniteElementSpace*>& fes,
...
int main( int argc, char* argv[] )
{
    ...
    args.AddOption( &newton_iter, "-it", "--newton-iterations", "Maximum iterations for the Newton solve." );
    args.AddOption( &mu, "-mu", "--shear-modulus", "Shear modulus for the neo-Hookean material." );
    args.Parse();
    if ( !args.Good() )
    {
    ...
    ParaViewDataCollection paraview_dc1( "test1", mesh );
    paraview_dc1.SetPrefixPath( "ParaView" );
    paraview_dc1.SetLevelsOfDetail( order );
    paraview_dc1.SetCycle( 0 );
    paraview_dc1.SetDataFormat( VTKFormat::BINARY );
    paraview_dc1.SetHighOrderOutput( true );
    paraview_dc1.SetTime( 0.0 ); // set the time
    paraview_dc1.RegisterField( "Displace", &x_def );
    paraview_dc1.Save();
    ...
    RubberOperator oper( spaces, ess_bdr, block_trueOffsets, newton_rel_tol, newton_abs_tol, newton_iter, c_mu );
...
JacobianPreconditioner::JacobianPreconditioner( Array<FiniteElementSpace*>& fes, SparseMatrix& mass, Array<int>& offsets )
      ...
      block_trueOffsets( offsets )
{
    ...
    JacobianPreconditioner* jac_prec = new JacobianPreconditioner( fes, *pressure_mass, block_trueOffsets );
    j_prec = jac_prec;
...
void visualize( ostream& out, Mesh* mesh, GridFunction* deformed_nodes, GridFunction* field, const char* field_name, bool init_vis )
{
    if ( !out )
    {
        return;
