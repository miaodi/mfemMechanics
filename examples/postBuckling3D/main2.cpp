...
int main( int argc, char* argv[] )
{
    ...
    args.AddOption( &f_stress_xx, "-fs", "--fstressXX", "Final stressXX." );
    args.Parse();
    if ( !args.Good() )
    {
        if ( myid == 0 )
    ...
    // intrinsic stress
    {
        auto intrinsic = new VectorArrayCoefficient( 6 );
        iem.setIntrinsicStress( intrinsic );

        Vector push_force( pmesh->attributes.Max() );
        push_force = f_stress_xx;
        intrinsic->Set( 0, new PWConstCoefficient( push_force ) );
        // intrinsic->Set( 1, new PWConstCoefficient( push_force ) );
        // intrinsic->Set( 2, new PWConstCoefficient( push_force ) );
    }
