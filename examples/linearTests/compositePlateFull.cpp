...
int main( int argc, char* argv[] )
{
    ...
    const char* mesh_file = "../../data/simply_support_composite_full.msh";
    int order = 1;
...

    {
        ...
        {
            if ( std::abs( mesh->GetVertex( i )[2] ) < 1e-10 )
            {
                if ( std::abs( mesh->GetVertex( i )[0] - .254 ) < 1e-10 || std::abs( mesh->GetVertex( i )[1] - .254 ) < 1e-10 ||
                     std::abs( mesh->GetVertex( i )[0] ) < 1e-10 || std::abs( mesh->GetVertex( i )[1] ) < 1e-10 )
                {
                    ess_tdof_list.Append( 0 * mesh->GetNV() + i );
                    ess_tdof_list.Append( 1 * mesh->GetNV() + i );
                    ess_tdof_list.Append( 2 * mesh->GetNV() + i );
                }
            }
        }
    }
    ...
    Vector top_force( mesh->bdr_attributes.Max() );
    top_force = .0;
    top_force( 24 ) = -689.4e3;
    f1.Set( 2, new PWConstCoefficient( top_force ) );
    nlf->AddBdrFaceIntegrator( new plugin::NonlinearVectorBoundaryLFIntegrator( f1 ) );

    ParaViewDataCollection paraview_dc( "plate_full", mesh );
